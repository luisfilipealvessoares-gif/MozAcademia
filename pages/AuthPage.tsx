...
const AuthPage: React.FC = () => {
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
      ...
      if (error) {
          ...
          setSuccessMessage('Se o e-mail estiver correto, um link para redefinir sua senha foi enviado.');
      }
      setLoading(false);
  };
        ...
        <>
            <div className="flex flex-col items-center">
                ...
                <h2 className="mt-6 text-3xl font-extrabold text-center text-gray-900">
                  ...
                  {view === 'register' && 'Crie uma nova conta'}
                  {view === 'reset' && 'Redefinir Senha'}
                </h2>
                 {view === 'reset' && <p className="mt-2 text-center text-sm text-gray-600">Digite seu e-mail para receber um link de redefinição.</p>}
            </div>
                     ...
                     {view === 'login' && (
                        <div className="flex items-center justify-end">
                            <div className="text-sm">
                                <button type="button" onClick={() => setView('reset')} className="font-medium text-brand-up hover:text-brand-moz">
                                    Esqueceu a senha?
                                </button>
                            </div>
                        </div>
            ...
            <div className="text-sm text-center">
                ...
                >
                    {view === 'login' && 'Não tem uma conta? Registre-se'}
                    {view === 'register' && 'Já tem uma conta? Faça login'}
                    {view === 'reset' && 'Lembrou da senha? Voltar para Login'}
                </button>
            </div>
        </>
        )}
